...
class LBRegistry:
    ...
    def register(
        ...
        """
        Add or refresh a healthy worker in the load-balancer pool.

        Uses UpdateItem with ``if_not_exists`` guards on counters and
        ``added_at`` so that a re-registration after an orchestrator restart
        preserves accumulated stats rather than resetting them to zero.
        """
    ...
    def deregister(self, worker_id: str) -> None:
        """Remove a worker from the load-balancer pool."""
        try:

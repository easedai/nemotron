...
# Max chars stored for a log snapshot (keep items well under DynamoDB's 400 KB limit)
...
class EventStore:
    ...
    Table schema
    ────────────
    PK  worker_id  (string)   — orchestrator worker ID
    SK  ts         (string)   — ISO timestamp (μs precision) — sorts chronologically
    TTL expires_at (number)   — Unix epoch seconds

    GSI  instance_id-index
         PK  instance_id  (string)   — vast.ai instance ID
         SK  ts           (string)

    Other attributes
    ────────────────
    event_type   string   — dotted name, e.g. "worker.created", "health.fail",
                            "status.changed", "log.snapshot"
    status       string   — WorkerStatus at the time of the event
    prev_status  string?  — previous status (status.changed events)
    instance_id  string?  — vast.ai instance ID
    label        string?  — vast.ai label (eased-<worker_id>)
    message      string   — human-readable description
    meta         string   — JSON blob with extra context (reason, specs, …)
    log_text     string?  — container log tail (log.snapshot events, ≤ 80 KB)
    """
    ...
    def record(
        ...
        """Write one lifecycle event.  Never raises — monitoring must not
    ...
    def record_logs(
        ...
        """
        Cache a vast.ai container log snapshot in the event table.
        `trigger` is a short string describing why logs were fetched
        (e.g. 'health_fail', 'terminated', 'periodic').
        Truncates to _MAX_LOG_CHARS so the item stays under DynamoDB's 400 KB limit.
        """
    ...
    def query_by_worker(self, worker_id: str, limit: int = 100) -> list[dict]:
        """Newest-first events for a given worker_id (primary key query)."""
        try:
    ...
    def query_by_label(self, label: str, limit: int = 100) -> list[dict]:
        """
        Resolve label → worker_id (label format: "eased-<worker_id>")
        then query by primary key.  Falls back to a scan if the prefix is
        missing (e.g. hand-crafted labels).
        """
        if label.startswith("eased-") and len(label) > 6:
            return self.query_by_worker(label[6:], limit)
        # Fallback: scan on the label attribute
        try:
    ...
    def query_by_instance(self, instance_id: str, limit: int = 100) -> list[dict]:
        """Query via the instance_id-index GSI."""
        try:
            resp = self.table.query(
    ...
    def _clean(items: list[dict]) -> list[dict]:
        """Decode meta JSON and strip internal TTL field."""
        for item in items:
            if "meta" in item:
                try:

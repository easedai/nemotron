...
class DynamoDB:
    ...
    def get_active_workers(self) -> list[Worker]:
        """Workers that are bidding, pending, starting, running, or unhealthy."""
        active = [
    ...
    def get_known_instance_ids(self) -> set[str]:
        """
        All instance_ids ever written to the table (any status).
        Used to detect orphaned vast.ai instances that have our label but
        were never saved — or were saved then terminated.
        Scans the GSI when available (cheaper — only indexed items present);
        falls back to a full table scan while the GSI is still backfilling.
        """
        try:
            resp = self.table.scan(
                IndexName="instance_id-index",
